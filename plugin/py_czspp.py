...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeVideoContent(self):
        rsp = self.fetch("https://www.c-zzy.com")
    ...
    def categoryContent(self, tid, pg, filter, extend):
        result = {}
        url = 'https://www.c-zzy.com/{0}/page/{1}'.format(tid, pg)
        rsp = self.fetch(url)
        root = self.html(self.cleanText(rsp.text))
    ...
    def detailContent(self, array):
        tid = array[0]
        url = 'https://www.c-zzy.com/movie/{0}.html'.format(tid)
        rsp = self.fetch(url)
        root = self.html(self.cleanText(rsp.text))
    ...
    def searchContent(self, key, quick):
        url = 'https://www.c-zzy.com/xssearch?q={0}'.format(key)
        rsp = self.fetch(url)
        root = self.html(self.cleanText(rsp.text))
    ...
    def playerContent(self, flag, id, vipFlags):
        url = 'https://www.c-zzy.com/v_play/{0}.html'.format(id)
